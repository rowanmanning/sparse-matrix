...
module.exports = class SparseMatrix {
	...
	_assertValidX(x) {
		if (!this.isValidX(x)) {
			throw new RangeError(`X must be an integer between ${this._firstCol} and ${this._lastCol}`);
		}
	}

	_assertValidY(y) {
		if (!this.isValidY(y)) {
			throw new RangeError(`Y must be an integer between ${this._firstRow} and ${this._lastRow}`);
		}
	}
